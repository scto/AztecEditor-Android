...
// This class was imported from AOSP and was modified to fit our needs, it's probably a good idea to
// keep it as a
// Java file.
...
public class Html {
  ...
  public interface VideoThumbnailGetter {
    void loadVideoThumbnail(
        String source, Html.VideoThumbnailGetter.Callbacks callbacks, int maxWidth);

    void loadVideoThumbnail(
        String source, Html.VideoThumbnailGetter.Callbacks callbacks, int maxWidth, int minWidth);
  ...
  public static Spanned fromHtml(
      String source, Context context, List<IAztecPlugin> plugins, List<String> ignoredTags) {
  ...
  public static Spanned fromHtml(
      ...
      boolean shouldIgnoreWhitespace) {

    Parser parser = new Parser();
    try {
      parser.setProperty(Parser.schemaProperty, HtmlParser.schema);
      parser.setFeature(
          Parser.rootBogonsFeature, false); // allows the unknown tags to exist without root element
    ...
    HtmlToSpannedConverter converter =
        new HtmlToSpannedConverter(
            source, tagHandler, parser, context, plugins, ignoredTags, shouldIgnoreWhitespace);
...
class HtmlToSpannedConverter implements org.xml.sax.ContentHandler, LexicalHandler {
  ...
  public Spanned convert() {
    ...
    Object[] paragraphs =
        spannableStringBuilder.getSpans(0, spannableStringBuilder.length(), ParagraphStyle.class);
    for (Object paragraph : paragraphs) {
      if (paragraph instanceof UnknownHtmlSpan
          || paragraph instanceof IAztecParagraphStyle
          || paragraph instanceof AztecMediaSpan) {
        continue;
      }
  ...
  private void handleStartTag(String tag, Attributes attributes, int nestingLevel) {
    ...
    if (tagHandler != null) {
      ...
      if (tagHandler.handleTag(
          true, tag, spannableStringBuilder, context, attributes, nestingLevel)) {
  ...
  private boolean handleContentStart(String tag, Attributes attributes) {
    if (contentHandlerLevel != 0) {
      ...
      content
          ...
          .append('<')
          .append(tag)
          .append(Html.stringifyAttributes(attributes))
          .append('>');
  ...
  private void handleEndTag(String tag, int nestingLevel) {
    ...
    if (tagHandler != null) {
      ...
      if (tagHandler.handleTag(
          false, tag, spannableStringBuilder, context, new AztecAttributes(), nestingLevel)) {
        return; // tag was handled
      }
    }
  ...
  private static void endFont(SpannableStringBuilder text) {
    ...
    if (font != null && where != len) {
      ...
      if (!TextUtils.isEmpty(color)) {
        if (color.startsWith("@")) {
          ...
          if (colorRes != 0) {
            ...
            text.setSpan(
                new TextAppearanceSpan(null, 0, 0, colors, null),
                where,
                len,
      ...
      if (face != null) {
        text.setSpan(new TypefaceSpan(face), where, len, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
      }
    }
  }

  private void endPluginContentHandler(
      SpannableStringBuilder text, int nestingLevel, ContentHandler unknown) {
  ...
  private void endContentHandler(
      SpannableStringBuilder text, int nestingLevel, ContentHandler unknown, Context context) {
    int len = text.length();
    ...
    if (where != len) {
      ...
      UnknownHtmlSpan unknownHtmlSpan =
          new UnknownHtmlSpan(
              nestingLevel, unknown.rawHtml, context, android.R.drawable.ic_menu_help);
    ...
    if (plugins != null) {
      for (IAztecPlugin plugin : plugins) {
        if (plugin instanceof IHtmlTextHandler) {
          ...
          while (matcher.find()) {
            boolean textHandled =
                textPlugin.onHtmlTextMatch(matcher.group(), spannableStringBuilder, nestingLevel);
            if (textHandled) {
